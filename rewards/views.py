...
class SpinWheelView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        RecentEarnings.objects.create(user=user, way_to_earn="Spin Wheel", point_earned=points)
        RecentEarnings.save()
...
class DailyCheckIn(APIView):
    ...
    def post(self, request, args, **kwargs):
        ...
        if last_check_in and timezone.now() - last_check_in.created_at < timedelta(days=1):
            return Response({'error': 'You have already earned points for a daily check-in in the last 24 hours.'}, status=400)
        ...
        return Response({'success': f'{50} points added to your account.'}, status=200)